...
def nbimageLCVbak2( data, vmin = None, vmax = None, vsym = False, saveas = None,
    ...
    # make colormap
    incr = (256//nshades)+1
    colors = [x for x in range(0,255,(256//nshades)+1)]
    colors[-1] = 255
    colctr = -1
    for i in range(256):
        if i % incr == 0:
...
'''
    ...
    for data in dlist:
        ...
        qim.save(buf, 'PNG')
        base64_data = ba.toBase64().data()
        # print('Type of ba.toBase64(): ', type(ba.toBase64()))
        # print('Type of ba.toBase64().data(): ', type(ba.toBase64().data()))
        # print(base64_data[0:10])

        if displayColCtr == 0:
            s += "<tr>"

        im = 'data:image/png;base64,' + base64_data.decode('utf-8')
